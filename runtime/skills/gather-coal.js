// gather.coal — find a nearby coal ore block, equip a pickaxe (best
// available), path to it and mine it. Modelled on gather.stone: stone-tier
// mining needs at least a wooden pickaxe, the preconditions enforce that.
//
// Why this exists: craft.torch hard-requires coal (or charcoal), but no other
// skill produces either, so M6_lighting was a permanent dead-end that pinned
// the Settlement Contract forever and starved M7/M8/M9. gather.coal is the
// missing producer. coal_ore drops coal directly and collectBlock awaits the
// pickup, so the strict ledger check (expectGain) is safe here.
...
const COAL_ORE_NAMES = ["coal_ore", "deepslate_coal_ore"];
...
const coalBlacklist = new WeakMap();
const BLACKLIST_TTL_MS = 5 * 60_000;

function getBlacklist(bot) {
	let m = coalBlacklist.get(bot);
	if (!m) {
		m = new Map();
		coalBlacklist.set(bot, m);
	}
	const now = Date.now();
...
export const skill = Object.freeze({
	...
	title: "Gather coal",
	timeoutMs: 90_000,
	// Closed-loop strict check: the milestone needs coal in hand, so we assert
	// the world actually moved (coal/charcoal increased) and refuse to report
	// "ok" if the dig left the inventory unchanged (runSkill → world_unchanged).
	expectGain: { matcher: (k) => k === "coal" || k === "charcoal", min: 1, label: "coal" },
	preconditions(ctx) {
	...
	async execute(ctx) {
		...
		const target = findNearestBlockByName(bot, COAL_ORE_NAMES, {
			maxDistance: 32,
			predicate: (b) => !blacklist.has(`${b.position.x},${b.position.y},${b.position.z}`),
		});
		if (!target) {
			return { ok: false, code: "no_target", detail: "no reachable coal ore within 32 blocks", worldDelta: null };
		}

		ensureCollectBlock(bot);
		setMovementsForGather(bot);
		const pickaxe = await equipBestPickaxe(bot);
		info("action", `gather.coal: ${target.name} at ${target.position.x},${target.position.y},${target.position.z} (tool=${pickaxe ?? "fists"})`);
		const targetPos = target.position.clone();
		try {
			try {
				await withTimeout(bot.lookAt(targetPos.offset(0.5, 0.5, 0.5), true), 2_000, "lookAt(coal)");
			} catch {}
			await withTimeout(bot.collectBlock.collect(target), 60_000, "collectCoal");
			const after = bot.blockAt(targetPos);
			if (after && COAL_ORE_NAMES.includes(after.name)) {
				warn("action", `gather.coal reported ok but block still at ${targetPos.x},${targetPos.y},${targetPos.z} — silent dig failure`);
			...
			warn("action", `gather.coal failed: ${e.message}`);
	...
	recover(ctx, result) {
		if (result.code === "no_target") return { hint: "wander", reason: "no coal ore within 32 blocks" };
		return null;
	},
});
